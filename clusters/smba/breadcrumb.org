...
kubectl create secret generic coder -n coder \
    --from-literal=dburl="postgres://coder:$coder_db_password@coder-db-postgresql.coder.svc.hackbach.cluster:5432/coder?sslmode=disable" \
...
W0329 22:43:37.430700   69346 helpers.go:663] --dry-run is deprecated and can be replaced with --dry-run=client.
...
kubectl create secret generic coder-github -n coder \
    ...
    --from-literal=oauth2-secret=$CODER_OAUTH2_GITHUB_CLIENT_ID \
